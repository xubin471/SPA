...
class Res101Encoder(nn.Module):
    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='ImageNet'):
        super().__init__()
        # using pretrained model's weights
        if pretrained_weights == 'COCO':
            self.pretrained_weights = torch.load(
                "./checkpoint/deeplabv3_resnet101_coco-586e9e4e.pth", map_location='cpu')  # pretrained on COCO
            print(f"load ResNet101-coco")
        elif pretrained_weights == 'ImageNet':
            self.pretrained_weights = torch.load(
                "./checkpoint/resnet101-63fe2227.pth", map_location='cpu')  # pretrained on ImageNet
            print("=================load resnet101-imagenet=================")
        else:
...
class Res50Encoder(nn.Module):
    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='COCO'):
        super().__init__()
        # using pretrained model's weights
        if pretrained_weights == 'COCO':
            self.pretrained_weights = torch.load(
                "./checkpoint/deeplabv3_resnet50_coco-cd0a2569.pth", map_location='cpu')  # pretrained on COCO
            print("==================load resnet50-coco========================")
        elif pretrained_weights == 'ImageNet':
            self.pretrained_weights = torch.load(
                "./checkpoint/resnet50-19c8e357.pth", map_location='cpu')  # pretrained on ImageNet
            print("==================load resnet50-imagenet========================")
        else:
            self.pretrained_weights = pretrained_weights
