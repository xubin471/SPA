...
class FewShot(nn.Module):
    ...
    def forward(self, sup_img,qry_img, sup_msk,qry_msk,mode="train"):
        ...
        # ================================================================================
        # sup_glob_proto, _ = self._extract_prototypes(sup_fts, sup_msk)  # [1 512]
        ...
        qry_pred = torch.argmax(qry_pred_logit,dim=1) #[1 256 256]
        if mode == "train":
            coarse_loss += self.loss.pred_loss(qry_pred_logit,qry_msk)
        # ================================================================================
        # qry_glob_proto, _ = self._extract_prototypes(qry_fts, qry_pred)  # [1 512]
        qry_glob_proto = self.GPG(qry_fts, qry_pred)  # [1 512]

        final_proto = self.CA(sup_glob_proto,qry_glob_proto)
        qry_sim = self.getPred(qry_fts, final_proto, self.qry_t) #[1 1 64 64]
        qry_pred = F.interpolate(qry_sim, size=img_size, mode='bilinear', align_corners=True) #[1 1 256 256]
        qry_pred_logit = torch.cat([1-qry_pred,qry_pred],dim=1) #[1 2 256 256]
        # ================================================================================
        if mode == "train":
            align_loss += self.align_loss(sup_fts_bak,qry_fts_bak,sup_msk,qry_pred_logit,shape_scores)
            pred_loss += self.loss.pred_loss(qry_pred_logit,qry_msk)
            # if torch.argmax(qry_pred_logit,dim=1).sum() > 1000:
            #     bd_shape_loss += 0.005 * self.loss.bd_shape_loss(qry_pred,sup_msk[None,...])
            #     consistency_loss += self.loss.pair_wise_consistency_loss(qry_pred_logit, qry_msk)
            # kl_shape_loss += self.loss.kl_shape_loss(qry_pred,sup_msk)
            return qry_pred_logit, pred_loss,align_loss+coarse_loss+consistency_loss, bd_shape_loss
        else:
    ...
    def align_loss(self, sup_fts, qry_fts, sup_msk, qry_pred,shape_scores1):
        ...
        if shape_scores1 is not None:
            align_loss += self.loss.l1_loss(shape_scores1,shape_scores)
        # # ================================================================================
        sup_fts = sup_fts * shape_scores[:,:,None,None]
        qry_fts = qry_fts * shape_scores[:,:,None,None]
        # ================================================================================
        # qry_glob_proto, _ = self._extract_prototypes(qry_fts, qry_pred_mask)  # [1 512]
        ...
        sup_pred = torch.argmax(sup_pred_logit,dim=1) #[1 256 256]
        # =================================================================================
        # sup_glob_proto, _ = self._extract_prototypes(sup_fts, sup_pred)  # [1 512]
        sup_glob_proto = self.GPG(sup_fts, sup_pred)  # [1 512]
        final_proto = self.CA(qry_glob_proto,sup_glob_proto)
        sup_sim = self.getPred(sup_fts, final_proto, self.sup_t) #[1 1 64 64]
        sup_pred = F.interpolate(sup_sim, size=sup_msk.shape[-2:], mode='bilinear', align_corners=True) #[1 1 256 256]
        sup_pred_logit = torch.cat([1-sup_pred,sup_pred],dim=1) #[1 2 256 256]
        # sup_pred = torch.argmax(sup_pred_logit,dim=1) #[1 256 256]
        # =================================================================================
    ...
    def _extract_prototypes(self, sup_fts, sup_msk):
        ...
        # for support set
        sup_fts_trans = F.interpolate(sup_fts, size=img_size, mode='bilinear',
                                      align_corners=True)  # (1 feat_dim 256 256)
        glob_proto = torch.sum(sup_fts_trans*sup_msk[None,...],dim=(-2,-1)) / (sup_msk.sum() + 1e-8)

        return glob_proto, None
