...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0, 'shape_loss': 0, 'b_loss': 0, 'edge_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            qry_pred_logit, query_loss,align_loss, shape_loss = model(support_images[0][0], query_images[0], support_fg_mask[0][0],query_labels, mode='train')
            loss = query_loss + align_loss + shape_loss
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, align_Loss:{align_loss}, shape_loss:{shape_loss}')

                # _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, shape_loss: {shape_loss}'
                #           f', b_loss: {b_loss}, edge_loss: {edge_loss}, proto_loss: {proto_loss}')
